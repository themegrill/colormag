...
* Fix      - Block editor paragraph and list text ignored a customized Base Typography font size.
* Fix      - H3 heading rendered at the same size as H2 in the block editor and front end.
* Fix      - Block editor heading, blockquote, link, and button colors didn't match the front end's palette color.
* Fix      - Header Builder Button's typography font wasn't loading in the block editor.
* Fix      - Search icon clipped when a custom icon size was set.
* Fix      - Current menu ancestor items in the primary navigation didn't inherit the active text color.
* Fix      - Imported demo footer widget text color was forced white, overriding the widget's own styling.
* Fix      - Header logo placement migration ran even on fresh installs that never had the legacy setting.
* Fix      - Footer bottom bar alignment (right/center) migration never applied due to a variable mismatch.
* Fix      - Header bottom area background color wasn't applying the selected global palette color.
* Fix      - Advanced blog post layout style 2 wasn't rendering the featured image on the right.
* Fix      - Scroll-to-top button auto-hid itself even when the page loaded already scrolled down.
...
* Enhance    - Less background work on every page load for better overall speed.
...
* Fix      - Border color issue in the header search input and "search again" field.
* Fix      - UTF-8 BOM in migration files triggering Quirks Mode and dashboard crashes.
