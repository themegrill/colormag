...
class ColorMag_Dynamic_CSS {
	...
	/**
	 * Escapes a CSS value and appends `!important`, unless it's empty.
	 *
	 * Used only for editor-only overrides that need to reliably beat static
	 * rules of identical specificity in style-editor-block.css (see the note
	 * in colormag_editor_block_css()). Leaves empty values empty so callers'
	 * existing "skip if empty" checks (in colormag_parse_css() etc.) still work.
	 *
	 * @param string $value Raw value.
	 * @return string
	 */
	private static function important_value( $value ) {
	...
	/**
	 * Appends `!important` to every declaration in a generated CSS block.
	 *
	 * For the same reason as important_value() above, used to make
	 * colormag_parse_dimension_css()/colormag_parse_slider_css() output reliably
	 * win in the editor iframe.
	 *
	 * @param string $css CSS generated by colormag_parse_dimension_css() or colormag_parse_slider_css().
	 * @return string
	 */
	private static function important_css( $css ) {
		return empty( $css ) ? '' : str_replace( ';', ' !important;', $css );
	}
	...
	public static function colormag_editor_block_css() {
		...
		// static rules of identical specificity in style-editor-block.css, and
		// the block editor iframe does not reliably preserve source order
		// between its cloned <link> stylesheets and injected inline <style>
		// blocks the way a normal document does — so a plain specificity/order
		// tie is not enough to guarantee these win. WordPress core takes the
		// same approach for its own preset color classes (e.g.
		// `.has-*-background-color { ... !important; }`) for the same reason.
		$parse_css .= colormag_parse_css(
			'#207daf',
			$primary_color,
			array(
				'.editor-styles-wrapper .wp-block-quote' => array(
					'background-color' => self::important_value( $primary_color ),
				),
			)
		);
		...
		$parse_css .= colormag_parse_css(
			...
			array(
				'.editor-styles-wrapper' => array(
					'color' => self::important_value( $base_color ),
				),
			)
		);
		...
		$parse_css .= colormag_parse_css(
			...
			array(
				'.editor-styles-wrapper a' => array(
					'color' => self::important_value( $link_color ),
				),
			)
		);
		...
		$parse_css       .= colormag_parse_css(
			...
			array(
				'.editor-styles-wrapper a:hover' => array(
					'color' => self::important_value( $link_hover_color ),
				),
			)
		);
		...
		$parse_css        .= colormag_parse_css(
			...
			array(
				$button_selector => array(
					'color' => self::important_value( $button_text_color ),
				),
			)
		);
		...
		$parse_css              .= colormag_parse_css(
			...
			array(
				$button_hover_selector => array(
					'color' => self::important_value( $button_hover_text_color ),
				),
			)
		);
		...
		$parse_css               .= colormag_parse_css(
			...
			array(
				$button_selector => array(
					'background-color' => self::important_value( $button_background_color ),
				),
			)
		);
		...
		$parse_css                     .= colormag_parse_css(
			...
			array(
				$button_hover_selector => array(
					'background-color' => self::important_value( $button_background_hover_color ),
				),
			)
		);
		...
		$parse_css .= self::important_css(
			colormag_parse_dimension_css(
				$button_padding_default,
				$button_padding,
				$button_selector,
				'padding'
